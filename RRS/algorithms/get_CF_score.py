...
class CF_score():
    ...
    def get_cf_score(self, x, y_range, top_k=1, a=0.3978):
        ...
        for y, score in zip(y_range.index, y_range.values) :
            ...
            for u in sent_to_y :
                try :
                    u_refrom = df[df['mem_no']==u].send_list.values[0] #u가 메시지를 보낸 유저집단(ReFrom_u)
                    x_refrom = df[df['mem_no']==x].send_list.values[0] #x가 메시지를 보낸 유저집단(ReFrom_x)
            ...
            for v in sent_to_x :
                try :
                    v_refrom = df[df['mem_no']==v].send_list.values[0] # v가 메시지를 보낸 유저집단(ReFrom_v)
                    y_refrom = df[df['mem_no']==y].send_list.values[0] # y가 메시지를 보낸 유저집단(ReFrom_y)
